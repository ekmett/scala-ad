...
class AD[S[_], A](val guts: S[A])(implicit val mode: Mode[S]) { 
...
object AD { 
  def apply[S[_]:Mode, A](value: S[A]) = new AD[S,A](value)

  class ADNumeric[S[_], A](implicit mode: Mode[S], implicit val A: Numeric[A]) extends Numeric[AD[S,A]] {
  ...
  implicit def ADIsNumeric[S[_]:Mode,A:Numeric] : Numeric[AD[S,A]] = new ADNumeric[S,A]()

  class ADFractional[S[_], A](implicit mode: Mode[S], override implicit val A: Fractional[A]) extends ADNumeric[S,A] with Fractional[AD[S,A]] {
    def div(a: AD[S, A], b: AD[S, A]): AD[S, A] = AD(mode.div(a.guts,b.guts))
  }

  implicit def ADIsFractional[S[_]:Mode,A:Fractional] : Fractional[AD[S,A]] = new ADFractional[S,A]()

  class ADFloating[S[_], A](implicit mode: Mode[S], override implicit val A: Floating[A]) extends ADFractional[S,A] with Floating[AD[S,A]] {
    ...
    // def asinh(a: AD[S,A]): AD[S,A] = AD(mode.asinh(a.guts))
    // def acosh(a: AD[S,A]): AD[S,A] = AD(mode.acosh(a.guts))
    // def atanh(a: AD[S,A]): AD[S,A] = AD(mode.atanh(a.guts))
  }

  implicit def ADIsFloating[S[_]:Mode, A:Floating]: Floating[AD[S,A]] = new ADFloating[S,A]()
